---
title: "Modi (10 days Data)Sentiment Analysis"
output:
...
We are using a dataset collected fro twitter for sentiment-analysis of political figure and prime minister- Narendra Modim.
...
dataframe=read.csv('D:/personal/ISB/pgdba/CBA/offer/CBA/Course/term 1/Practicum/Modi_df_1.csv', stringsAsFactors = F)
df=dataframe$text[sample(nrow(dataframe), 4000) ]


head(df,5) #uncleaned data
...
stpw1 = readLines('D:/personal/ISB/pgdba/CBA/offer/CBA/Course/term 1/Practicum/current df/stopwords.txt')

# tm package stop word list; tokenizer package has the same name function, hence 'tm::'
stpw2 = tm::stopwords('english')      
comn  = unique(c(stpw1, stpw2,'modi','narendra','ji','modiji','narendramodi','sir','indian','pm','NaMo'))      # Union of the two lists
stopwords = unique(gsub("'"," ",comn))  # final stop word list after removing punctuation

# removing stopwords created above
df  =  removeWords(df,stopwords)        	 
...
```{r wordcl}
 ...
 wordcloud(names(tsum), tsum,     # words, their freqs 
          scale = c(3.0, 0.4),     # range of word sizes
...
```{r func4}
 ...
 # system.time({ plot.barchart(dtm_df_idf, num_tokens=12, fill_color="Red") })    # 0.11 secs
``` 
...
```{r cog}
 ...
 system.time({ distill.cog(dtm_df_tf, "COG for df TF") })   
 system.time({ distill.cog(dtm_df_idf, "COG for df IDF", 5, 5) }) 
...
head(a)
```
To see joyful words most occurred in the corpus. 
...
To see angry words most occurred in the corpus. 
```{r df.anger}
df_anger = textdf %>%
      unnest_tokens(word, text) %>%
      inner_join(nrc) %>%
